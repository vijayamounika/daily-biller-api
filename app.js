...
function getSpecificDate(req, res, next) {
    ...
    db.collection('dailySubscriptions').find({ "subId": subId, "date": { "$gte": new Date(str1), "$lt": new Date(str) } }).toArray(function (err, result) {
        if (err) throw new Error("Error");
        if (result) {
        ...
        if (docs) {
            ...
            if (docs.subType == "daily") {
                if (req.body.priceperunit) {
                    cost = req.body.priceperunit * quantity;
                } else {
                    throw new Error("please enter price per unit");
                }

            } else cost = docs.pricePerUnit * quantity;
            ...
            dateString = 2018 + "-" + "0" + 9 + "-" + 26 + "T00:00:00.000Z";
            ...
            db.collection('dailySubscriptions').find({ "$and": [{ "$expr": { "$eq": ["$date", mydate] } }, { "name": prName }] }).toArray(function (err, result) {
                // db.collection('dailySubscriptions').find({ "name": prName, "date": day }).toArray(function (err, result) {
                if (err) throw new Error("Error");
                if (result.length > 0) res.send("Cannot create two daily subscriptions on a single date, please update to change");
                else {
                    db.collection('dailySubscriptions').insertOne(myDailySubscription, function (err, result) {
                        if (err) throw new Error("error");
                        if (result) {
...
app.post('/auth/user/getSpecificDateSubscriptionAndUpdate', validateSubscriptionId, getSpecificDate, function (req, res) {
    var quantity = req.body.quantity;
    db.collection('dailySubscriptions').update({ "_id": req.recordOnDate[0]._id }, { $set: { "quantity": quantity } }, function (err, result) {
        if (err) throw new Error("error");
        if (result)
            res.send("updated successfully");
        else throw new Error("error");
...
app.post('/auth/user/getSpecificDateSubscriptionAndDelete', validateSubscriptionId, getSpecificDate, function (req, res) {
    db.collection('dailySubscriptions').remove({ "_id": req.recordOnDate[0]._id });
    res.send("Deleted successfully");
})
...
app.post('/auth/user/getSpecifiedDateSubscriptions', validateSubscriptionId, getSpecificDate, function (req, res) {
    if (req.user) {
